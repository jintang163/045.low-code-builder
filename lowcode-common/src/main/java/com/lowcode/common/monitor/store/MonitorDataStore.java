...
public class MonitorDataStore {
    ...
    public static MonitorMetrics getMetrics() {
        ...
        long reqTotal = requestLogs.stream()
                .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(todayStart))
                .count();
        metrics.setRequestTotal(reqTotal);

        long errTotal = requestLogs.stream()
                .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(todayStart))
        ...
        OptionalDouble avgTime = requestLogs.stream()
                .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(todayStart))
        ...
        long qps = requestLogs.stream()
                .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(oneMinuteAgo))
                .count() / 60;
        metrics.setQps(qps);
        ...
        List<Map<String, Object>> apiTop10 = apiStats.entrySet().stream()
                .sorted((a, b) -> Long.compare(b.getValue()[1] / Math.max(b.getValue()[0], 1),
    ...
    private static List<Map<String, Object>> buildTrendData(List<?> logs, String timeField, int days) {
        ...
        for (int i = days - 1; i >= 0; i--) {
            ...
            long count = logs.stream()
                    .filter(l -> {
                        try {
                            LocalDateTime t = (LocalDateTime) l.getClass().getMethod("get" +
                                    timeField.substring(0, 1).toUpperCase() + timeField.substring(1)).invoke(l);
                            return t != null && t.isAfter(start) && t.isBefore(end);
                        } catch (Exception e) {
                            return false;
                        }
                    })
    ...
    private static List<Map<String, Object>> buildErrorTrend(int days) {
        ...
        for (int i = days - 1; i >= 0; i--) {
            ...
            LocalDateTime start = LocalDateTime.of(date, LocalTime.MIN);
            LocalDateTime end = LocalDateTime.of(date, LocalTime.MAX);
            long total = requestLogs.stream()
                    .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(start) && l.getRequestTime().isBefore(end))
                    .count();
            long errors = requestLogs.stream()
                    .filter(l -> l.getRequestTime() != null && l.getRequestTime().isAfter(start) && l.getRequestTime().isBefore(end))
                    .filter(l -> l.getResponseStatus() != null && l.getResponseStatus() >= 500)
                    .count();
            Map<String, Object> m = new HashMap<>();
    ...
    private static List<Map<String, Object>> buildServiceStatus() {
        ...
        String[] allServices = {"lowcode-auth", "lowcode-model", "lowcode-page", "lowcode-flow",
                "lowcode-generator", "lowcode-oss", "lowcode-gateway"};
        ...
        for (String svc : allServices) {
            ...
            m.put("status", stats[0] == 0 ? "unknown" : (stats[1] * 100.0 / stats[0] > 10 ? "warning" : "healthy"));
            result.add(m);
        }
        return result;
    }
    ...
    private static void checkAlertRules(RequestLog requestLog) {
        for (AlertRule rule : alertRules) {
            if (!rule.getEnabled()) continue;
            if ("error_rate".equals(rule.getRuleType())) {
                checkErrorRateAlert(rule);
            } else if ("slow_api".equals(rule.getRuleType())) {
                checkSlowApiAlert(rule, requestLog);
            }
        }
    }

    private static void checkErrorRateAlert(AlertRule rule) {
        LocalDateTime startTime = LocalDateTime.now().minusMinutes(Math.max(rule.getDuration(), 1));
        List<RequestLog> recentLogs = requestLogs.stream()
        ...
        if (">".equals(rule.getOperator()) && errorRate > rule.getThreshold()) trigger = true;
        else if (">=".equals(rule.getOperator()) && errorRate >= rule.getThreshold()) trigger = true;
        else if ("<".equals(rule.getOperator()) && errorRate < rule.getThreshold()) trigger = true;
        else if ("<=".equals(rule.getOperator()) && errorRate <= rule.getThreshold()) trigger = true;

        if (trigger) {
            triggerAlert(rule, "错误率告警: 当前错误率 " + String.format("%.2f", errorRate) + "% 超过阈值 " + rule.getThreshold() + "%");
        }
    }

    private static void checkSlowApiAlert(AlertRule rule, RequestLog log) {
        if (log.getCostTime() != null && log.getCostTime() > rule.getThreshold()) {
            triggerAlert(rule, "慢接口告警: " + log.getRequestUrl() + " 耗时 " + log.getCostTime() + "ms 超过阈值 " + rule.getThreshold() + "ms");
        }
    }
